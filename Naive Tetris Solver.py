...
class Tetris:
    ...
    def calculate_game_fitness(self, list_of_self):
        for game_state in list_of_self:
            ...
            holes = 0
            width_sum = 0
            bottom_coverage = 0  # Tracks how much of the bottom is covered

            # Calculate width, holes, and bottom coverage
                ...
                for col in range(width):
                    if field[row][col] > 0:
                        ...
                        if not bottom_covered:
                            bottom_coverage += height - row
                            bottom_covered = True
                    elif col < width - 1 and field[row][col + 1] == 0:
                        holes_in_row += 1
                holes += holes_in_row
                width_sum += row_width
            ...
            fitness = (width_sum / total_blocks) - (holes / total_blocks) + (bottom_coverage / (width * height))
            print(f"width: {width_sum}   total blocks: {total_blocks}   bottom_coverage: {bottom_coverage}  fitness: {fitness}")

            game_state.fitness = fitness

        # Sort the list of grids by fitness values
        return sorted(list_of_self, key=lambda x: x.fitness, reverse=True)
    ...
    def generate_game_states(self):
        """
        Generate all possible game states resulting from valid figure placements.
        """
        valid_placements = self.placeable()
        game_states = []
        for fig in valid_placements:
            new_game_state = copy.deepcopy(self)
            new_game_state.figure = fig
            new_game_state.freeze()
            game_states.append(new_game_state)
            
        return game_states

    def select_random_state_game(self):
        """
        Randomly select a game state from the list of generated game states.
        """
        game_states = self.generate_game_states()
        # Calculate fitness scores for each game state
        fitness_scores = self.calculate_game_fitness(game_states)

        # Limit fitness scores to only the best 4
        fitness_scores = sorted(fitness_scores, key=lambda x: x.fitness)[:4]

        # If there are at least 4 game states, randomly select one from the best 4
        if fitness_scores:
            return_game = random.choice(fitness_scores)
            return_game = fitness_scores[0]
        else:
            return_game = self
...
if __name__ == '__main__':
    ...
    while not done:
        ...
        game = game.select_random_state_game()
        game.figure = None
        time.sleep(2)
